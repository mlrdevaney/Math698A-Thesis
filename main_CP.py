...
#path = 'C:\\Users\\ama001\\work\\proposals\\SFFP19\\Simulations_Learning_Collision\\Take1\\results\\'
...
sol, solsize = my_readwrite.my_read_solution_trim(filepath,MM, Mtrim)  # this is the first file
...
output_folder = 'moments'
os.makedirs(output_folder, exist_ok=True)
output_filename = f'{run_id}_moments_{fin_time_str}.txt'
start = time.time()
count = 0
############################################################################
#############################################################################
...
while running < fin_time:
    ...
    count += 1
    if count % 100 == 0:
        print(f"average time for past {count} iterations: {(time.time() - start)/count}(sec) after {count} iterations")
    ffm = sol + maxwell
    delta_f = sol - maxwell
    ############################################################################
    #############################################################################
    ...
    coll_oper = boltzcol2(ffm, delta_f, MM, rank)
...
print("Time of Looping:", time.time()-start)
